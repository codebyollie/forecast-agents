"""
GreyClaw v0.3 benchmark: full outer-loop training with scheduler.
...
async def run_training():
    from greyclaw.config import GreyClawConfig
    from greyclaw.trainer import GreyClawTrainer
    from greyclaw.scheduler import SlowUpdateScheduler
    from greyclaw.idle_detector import IdleDetector, LastRequestTracker
...
async def run_evaluation(config):
    ...
    from greyclaw.bedrock_client import BedrockChatClient
    from greyclaw.prm_scorer import PRMScorer
...
async def main():
    logger.info("=" * 60)
    logger.info("  GreyClaw v0.3 Benchmark")
