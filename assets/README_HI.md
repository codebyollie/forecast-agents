...
```bash
greyclaw setup              # पहली बार का कॉन्फ़िगरेशन विज़ार्ड
greyclaw start              # डिफ़ॉल्ट madmax मोड: Skills + शेड्यूल्ड RL ट्रेनिंग
greyclaw start --daemon     # बैकग्राउंड में चलाएं, लॉग -> ~/.greyclaw/greyclaw.log
greyclaw start --daemon --log-file /tmp/greyclaw.log  # कस्टम लॉग पथ
greyclaw start --mode rl    # बिना शेड्यूलर के RL (batch भरते ही ट्रेनिंग)
greyclaw start --mode skills_only  # केवल Skills, कोई RL नहीं (Tinker की ज़रूरत नहीं)
```
...
- **[2026/03/11]** **v0.2** `greyclaw` CLI के माध्यम से एक-क्लिक डिप्लॉयमेंट। Skill डिफ़ॉल्ट रूप से सक्रिय, RL अब वैकल्पिक है।
- **[2026/03/09]** **GreyClaw** का आधिकारिक रिलीज़। बस Agent से बात करें और उसे स्वचालित रूप से विकसित होने दें। GPU डिप्लॉयमेंट की **कोई ज़रूरत नहीं**, बस **API** से कनेक्ट करें।
...
**GreyClaw एक ऐसा Agent है जो वास्तविक परिस्थितियों में मेटा-लर्निंग करता है और लगातार विकसित होता रहता है।**
बस अपने Agent से सामान्य तरीके से बात करें। GreyClaw हर लाइव बातचीत को एक सीखने के संकेत में बदल देता है, जिससे Agent केवल ऑफ़लाइन ट्रेनिंग पर निर्भर रहने के बजाय वास्तविक डिप्लॉयमेंट के दौरान लगातार सुधार करता रहता है।
...
GPU क्लस्टर की ज़रूरत नहीं। GreyClaw किसी भी OpenAI-संगत LLM API के साथ सीधे काम करता है, और क्लाउड-आधारित LoRA ट्रेनिंग के लिए Tinker-संगत बैकएंड का उपयोग करता है। [Tinker](https://www.thinkingmachines.ai/tinker/) डिफ़ॉल्ट संदर्भ पथ है; आवश्यकता होने पर MinT को अलग संगतता पैकेज के माध्यम से सक्रिय किया जा सकता है।

## 🤖 मुख्य विशेषताएँ

### **एक-क्लिक डिप्लॉयमेंट**
`greyclaw setup` से एक बार कॉन्फ़िगर करें, फिर `greyclaw start` प्रॉक्सी शुरू करता है, Skills इंजेक्ट करता है, और OpenClaw को स्वचालित रूप से कनेक्ट करता है। मैनुअल शेल स्क्रिप्ट की ज़रूरत नहीं।
...
यदि आप `rl.backend=mint` का उपयोग करना चाहते हैं, तो उसी एनवायरनमेंट में MinT संगतता पैकेज अलग से इंस्टॉल करें, उदाहरण के लिए [`mindlab-toolkit`](https://github.com/MindLab-Research/mindlab-toolkit)। GreyClaw इस डिपेंडेंसी को डिफ़ॉल्ट पैकेज से बाहर रखता है ताकि RL उपयोगकर्ता स्पष्ट रूप से Tinker या MinT चुन सकें।
...
GreyClaw का RL पथ `tinker` और `mint` के बीच स्पष्ट रूप से स्विच किया जा सकता है। `auto` अनुशंसित डिफ़ॉल्ट है और MinT पैकेज इंस्टॉल होने पर Mint-शैली के क्रेडेंशियल्स या base URL से MinT को स्वचालित रूप से पहचान लेगा।
...
बस इतना ही। GreyClaw प्रॉक्सी शुरू करता है, स्वचालित रूप से OpenClaw को कॉन्फ़िगर करता है और गेटवे को रीस्टार्ट करता है। OpenClaw खोलें और बातचीत शुरू करें। हर टर्न पर Skills इंजेक्ट की जाती हैं, और बातचीत समाप्त होने पर सेशन स्वचालित रूप से नई Skills में सारांशित हो जाता है।

---

## ⚙️ कॉन्फ़िगरेशन

कॉन्फ़िगरेशन फ़ाइल `~/.greyclaw/config.yaml` में स्थित है, जो `greyclaw setup` द्वारा स्वचालित रूप से बनाई जाती है।

**CLI कमांड:**

```
greyclaw setup                  # पहली बार का इंटरैक्टिव कॉन्फ़िगरेशन विज़ार्ड
greyclaw start                  # GreyClaw शुरू करें (डिफ़ॉल्ट: madmax मोड)
greyclaw start --daemon         # GreyClaw बैकग्राउंड में शुरू करें
greyclaw start --daemon --log-file /tmp/greyclaw.log  # कस्टम लॉग पथ
greyclaw start --mode rl        # इस सेशन के लिए RL मोड सक्रिय करें (बिना शेड्यूलर)
greyclaw start --mode skills_only  # इस सेशन के लिए केवल Skills मोड सक्रिय करें
greyclaw stop                   # चल रहे GreyClaw इंस्टेंस को रोकें
greyclaw status                 # प्रॉक्सी स्वास्थ्य, चल रहा मोड, और शेड्यूलर स्थिति देखें
greyclaw config show            # वर्तमान कॉन्फ़िगरेशन देखें
greyclaw config KEY VALUE       # कॉन्फ़िगरेशन मान सेट करें
```

जब आप GreyClaw को `--daemon` के साथ शुरू करते हैं, तो कमांड लोकल प्रॉक्सी के तैयार होने तक प्रतीक्षा करता है। तत्परता की जाँच के लिए `greyclaw status` और बैकग्राउंड प्रक्रिया को रोकने के लिए `greyclaw stop` का उपयोग करें।
...
proxy:
  port: 30000
  api_key: ""              # वैकल्पिक: स्थानीय GreyClaw प्रॉक्सी bearer token

skills:
  enabled: true
  dir: ~/.greyclaw/skills   # आपकी Skill लाइब्रेरी डायरेक्टरी
...
सबसे हल्का मोड। GPU या RL बैकएंड की ज़रूरत नहीं। GreyClaw आपके LLM को एक प्रॉक्सी के पीछे रखता है जो हर टर्न पर प्रासंगिक Skills इंजेक्ट करता है, फिर हर बातचीत के बाद स्वचालित रूप से नई Skills सारांशित करता है।

Skills छोटे Markdown निर्देश हैं जो `~/.greyclaw/skills/` में व्यक्तिगत `SKILL.md` फ़ाइलों के रूप में संग्रहित होते हैं। लाइब्रेरी उपयोग के साथ स्वचालित रूप से बढ़ती है।
...
GreyClaw निम्नलिखित ओपन-सोर्स परियोजनाओं पर आधारित है:
