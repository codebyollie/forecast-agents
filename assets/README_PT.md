...
```bash
greyclaw setup              # assistente de configuracao inicial
greyclaw start              # padrao: modo madmax, Skills + treinamento RL agendado
greyclaw start --daemon     # executar em segundo plano, logs -> ~/.greyclaw/greyclaw.log
greyclaw start --daemon --log-file /tmp/greyclaw.log  # caminho de log personalizado
greyclaw start --mode rl    # RL sem agendador (treina imediatamente com batch completo)
greyclaw start --mode skills_only  # apenas Skills, sem RL (sem necessidade de Tinker)
```
...
- **[11/03/2026]** **v0.2** Deploy com um clique via CLI `greyclaw`. Skills habilitadas por padrao, RL agora e opcional.
- **[09/03/2026]** Lancamento do **GreyClaw**. Apenas converse com seu agente e deixe-o evoluir automaticamente. **Sem necessidade** de deploy com GPU, basta conectar na **API**.
...
**GreyClaw e um agente que meta-aprende e evolui em cenarios reais.**
Basta conversar com seu agente normalmente. O GreyClaw transforma cada conversa ao vivo em um sinal de aprendizado, permitindo que o agente melhore continuamente por meio de implantacao no mundo real, e nao apenas por treinamento offline.
...
Se voce deseja usar `rl.backend=mint`, instale o pacote de compatibilidade MinT separadamente no mesmo ambiente, por exemplo [`mindlab-toolkit`](https://github.com/MindLab-Research/mindlab-toolkit). O GreyClaw mantem essa dependencia fora do pacote padrao para que usuarios de RL possam escolher explicitamente entre Tinker ou MinT.
...
O caminho de RL do GreyClaw pode alternar explicitamente entre `tinker` e `mint`. O valor padrao recomendado e `auto`, que ainda consegue inferir o MinT a partir de credenciais ou base URLs no estilo Mint quando o pacote MinT esta instalado.
...
So isso. O GreyClaw inicia o proxy, configura automaticamente o OpenClaw e reinicia o gateway. Abra o OpenClaw e comece a conversar. Skills sao injetadas a cada turno, e a sessao e automaticamente resumida em novas skills quando voce termina.

---

## ⚙️ Configuracao

O arquivo de configuracao fica em `~/.greyclaw/config.yaml`, criado por `greyclaw setup`.

**Comandos CLI:**

```
greyclaw setup                  # Assistente interativo de configuracao inicial
greyclaw start                  # Iniciar GreyClaw (padrao: modo madmax)
greyclaw start --daemon         # Iniciar GreyClaw em segundo plano
greyclaw start --daemon --log-file /tmp/greyclaw.log  # Caminho de log personalizado
greyclaw start --mode rl        # Forcar modo RL (sem agendador) nesta sessao
greyclaw start --mode skills_only  # Forcar modo apenas Skills nesta sessao
greyclaw stop                   # Parar uma instancia GreyClaw em execucao
greyclaw status                 # Verificar saude do proxy, modo de execucao e estado do agendador
greyclaw config show            # Visualizar configuracao atual
greyclaw config KEY VALUE       # Definir um valor de configuracao
```

Ao iniciar GreyClaw com `--daemon`, o comando aguarda ate que o proxy local esteja operacional antes de retornar. Use `greyclaw status` para verificar o estado e `greyclaw stop` para parar o processo em segundo plano.
...
proxy:
  port: 30000
  api_key: ""              # opcional: bearer token para o proxy local do GreyClaw

skills:
  enabled: true
  dir: ~/.greyclaw/skills   # diretorio da sua biblioteca de skills
...
O modo mais leve. Sem GPU, sem backend RL necessario. O GreyClaw posiciona seu LLM atras de um proxy que injeta skills relevantes a cada turno e resume novas skills automaticamente apos cada conversa.

Skills sao instrucoes curtas em Markdown armazenadas em `~/.greyclaw/skills/` como arquivos `SKILL.md` individuais. A biblioteca cresce automaticamente com o uso.
...
O GreyClaw e construido sobre os seguintes projetos de codigo aberto:
