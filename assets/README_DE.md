...
```bash
greyclaw setup              # Einmaliger Konfigurationsassistent
greyclaw start              # Standard: MadMax-Modus, Skills + geplantes RL-Training
greyclaw start --daemon     # im Hintergrund starten, Logs -> ~/.greyclaw/greyclaw.log
greyclaw start --daemon --log-file /tmp/greyclaw.log  # benutzerdefinierter Log-Pfad
greyclaw start --mode rl    # RL ohne Scheduler (trainiert sofort bei vollem Batch)
greyclaw start --mode skills_only  # Nur Skills, kein RL (kein Tinker nötig)
```
...
- **[11.03.2026]** **v0.2** Ein-Klick-Deployment über `greyclaw` CLI. Skills standardmäßig aktiviert, RL jetzt optional.
- **[09.03.2026]** **GreyClaw** veröffentlicht. Sprich einfach mit deinem Agenten und lass ihn automatisch weiterentwickeln. **Kein** GPU-Deployment erforderlich; einfach an die **API** anschließen.
...
**GreyClaw ist ein Agent, der in realen Einsatzszenarien meta-lernt und sich weiterentwickelt.**
Sprich einfach wie gewohnt mit deinem Agenten. GreyClaw verwandelt jedes Live-Gespräch in ein Lernsignal und ermöglicht dem Agenten, sich durch den realen Einsatz kontinuierlich zu verbessern, statt nur auf Offline-Training zu setzen.
...
Kein GPU-Cluster nötig. GreyClaw funktioniert mit jeder OpenAI-kompatiblen LLM-API und nutzt ein Tinker-kompatibles Backend für Cloud-basiertes LoRA-Training. [Tinker](https://www.thinkingmachines.ai/tinker/) ist der Standard-Referenzpfad; bei Bedarf kann MinT über ein separates Kompatibilitätspaket aktiviert werden.

## 🤖 Hauptfunktionen

### **Ein-Klick-Deployment**
Einmal mit `greyclaw setup` konfigurieren, dann startet `greyclaw start` den Proxy, injiziert Skills und verbindet OpenClaw automatisch. Keine manuellen Shell-Skripte nötig.
...
Wenn du `rl.backend=mint` verwenden willst, installiere das MinT-Kompatibilitätspaket separat in derselben Umgebung, zum Beispiel [`mindlab-toolkit`](https://github.com/MindLab-Research/mindlab-toolkit). GreyClaw hält diese Abhängigkeit absichtlich aus dem Standardpaket heraus, damit RL-Nutzer Tinker oder MinT explizit wählen können.
...
Der RL-Pfad von GreyClaw kann explizit zwischen `tinker` und `mint` wechseln. `auto` ist die empfohlene Voreinstellung und kann MinT weiterhin aus Mint-ähnlichen Credentials oder Base-URLs ableiten, wenn das MinT-Paket installiert ist.
...
Das war's. GreyClaw startet den Proxy, konfiguriert OpenClaw automatisch und startet das Gateway neu. Öffne OpenClaw und beginne zu chatten. Skills werden bei jedem Schritt injiziert, und die Session wird automatisch zu neuen Skills zusammengefasst, wenn du fertig bist.

---

## ⚙️ Konfiguration

Die Konfiguration liegt in `~/.greyclaw/config.yaml`, erstellt durch `greyclaw setup`.

**CLI-Befehle:**

```
greyclaw setup                  # Interaktiver Erstkonfigurations-Assistent
greyclaw start                  # GreyClaw starten (Standard: MadMax-Modus)
greyclaw start --daemon         # GreyClaw im Hintergrund starten
greyclaw start --daemon --log-file /tmp/greyclaw.log  # Benutzerdefinierter Log-Pfad
greyclaw start --mode rl        # RL-Modus für diese Session erzwingen (ohne Scheduler)
greyclaw start --mode skills_only  # Nur-Skills-Modus für diese Session erzwingen
greyclaw stop                   # Laufende GreyClaw-Instanz stoppen
greyclaw status                 # Proxy-Status, laufenden Modus und Scheduler prüfen
greyclaw config show            # Aktuelle Konfiguration anzeigen
greyclaw config KEY VALUE       # Konfigurationswert setzen
```

Wenn Sie GreyClaw mit `--daemon` starten, wartet der Befehl, bis der lokale Proxy bereit ist, bevor er zurückkehrt. Verwenden Sie `greyclaw status` zur Überprüfung und `greyclaw stop` zum Stoppen des Hintergrundprozesses.
...
proxy:
  port: 30000
  api_key: ""              # optionales Bearer-Token für den lokalen GreyClaw-Proxy

skills:
  enabled: true
  dir: ~/.greyclaw/skills   # deine Skill-Bibliothek
...
Der leichteste Modus. Kein GPU, kein RL-Backend nötig. GreyClaw kapselt dein LLM hinter einem Proxy, der bei jedem Schritt relevante Skills injiziert und nach jedem Gespräch automatisch neue Skills zusammenfasst.

Skills sind kurze Markdown-Anweisungen in `~/.greyclaw/skills/` als einzelne `SKILL.md`-Dateien. Die Bibliothek wächst automatisch mit der Nutzung.
...
GreyClaw baut auf folgenden Open-Source-Projekten auf:
