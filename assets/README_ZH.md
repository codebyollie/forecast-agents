...
```bash
greyclaw setup              # 首次配置向导
greyclaw start              # 默认 madmax 模式：Skills + 定时 RL 训练
greyclaw start --daemon     # 后台运行，日志 -> ~/.greyclaw/greyclaw.log
greyclaw start --daemon --log-file /tmp/greyclaw.log  # 自定义日志路径
greyclaw start --mode rl    # 无调度器 RL（batch 满即训练）
greyclaw start --mode skills_only  # 仅 Skills，无 RL（无需 Tinker）
```
...
- **[2026/03/11]** **v0.2** 通过 `greyclaw` CLI 一键部署。Skill 默认开启,RL 现为可选。
- **[2026/03/09]** 正式发布 **GreyClaw**,只需与 Agent 对话,即可让其自动进化。**无需** GPU 部署,直接接入 **API** 即可。
...
**GreyClaw 是一个在真实场景中元学习并持续进化的 Agent。**
只需像平时一样与 Agent 对话, GreyClaw 将每一次实时对话转化为学习信号,让 Agent 在真实部署中持续进化,而非仅依赖离线训练。
...
无需 GPU 集群。GreyClaw 兼容任意 OpenAI 格式的 LLM API,并通过 Tinker 兼容后端进行云端 LoRA 微调。[Tinker](https://www.thinkingmachines.ai/tinker/) 是默认参考路径；如果需要,也可以通过单独安装的兼容包接入 MinT。

## 🤖 核心功能

### **一键部署**
使用 `greyclaw setup` 完成一次性配置,再执行 `greyclaw start` 即可自动启动代理、注入 Skill 并接入 OpenClaw。无需手动编写 Shell 脚本。
...
如果你要使用 `rl.backend=mint`,请在同一环境里额外安装 MinT 兼容包,例如 [`mindlab-toolkit`](https://github.com/MindLab-Research/mindlab-toolkit)。GreyClaw 不会把这个依赖放进默认安装中,这样 RL 用户可以明确选择 Tinker 或 MinT。
...
GreyClaw 的 RL 路径可以显式切换 `tinker` 和 `mint`。推荐默认值是 `auto`；当环境里安装了 MinT 兼容包时,它仍然可以根据 Mint 风格的凭证或 base URL 自动识别 MinT。
...
就这些。GreyClaw 启动代理,自动配置 OpenClaw 并重启网关。打开 OpenClaw 开始对话,每轮都会注入 Skill,对话结束后自动总结为新 Skill。

---

## ⚙️ 配置说明

配置文件位于 `~/.greyclaw/config.yaml`,由 `greyclaw setup` 自动生成。

**CLI 命令：**

```
greyclaw setup                  # 首次交互式配置向导
greyclaw start                  # 启动 GreyClaw（默认 madmax 模式）
greyclaw start --daemon         # 在后台启动 GreyClaw
greyclaw start --daemon --log-file /tmp/greyclaw.log  # 自定义日志路径
greyclaw start --mode rl        # 本次会话强制启用 RL 模式（无调度器）
greyclaw start --mode skills_only  # 本次会话强制仅 Skills 模式
greyclaw stop                   # 停止正在运行的 GreyClaw 实例
greyclaw status                 # 查看代理健康状态、运行模式与调度器状态
greyclaw config show            # 查看当前配置
greyclaw config KEY VALUE       # 设置配置项
```

使用 `--daemon` 启动 GreyClaw 时，命令会等待本地代理就绪后才返回。使用 `greyclaw status` 检查状态，使用 `greyclaw stop` 停止后台进程。
...
proxy:
  port: 30000
  api_key: ""              # 可选：本地 GreyClaw 代理 bearer token

skills:
  enabled: true
  dir: ~/.greyclaw/skills   # 你的 Skill 库目录
...
最轻量的模式。无需 GPU,无需 RL 后端。GreyClaw 将你的 LLM 封装在代理后面,每轮注入相关 Skill,对话结束后自动总结新 Skill。

Skill 是存放在 `~/.greyclaw/skills/` 中的简短 Markdown 指令,以独立的 `SKILL.md` 文件组织。Skill 库随使用自动增长。
