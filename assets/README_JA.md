...
```bash
greyclaw setup              # 初回設定ウィザード
greyclaw start              # デフォルト: madmax モード — スキル + スケジュール RL トレーニング
greyclaw start --daemon     # バックグラウンドで実行、ログ -> ~/.greyclaw/greyclaw.log
greyclaw start --daemon --log-file /tmp/greyclaw.log  # カスタムログパス
greyclaw start --mode rl    # スケジューラなし RL（バッチが満たされ次第即時トレーニング）
greyclaw start --mode skills_only  # スキルのみ、RL なし（Tinker 不要）
```
...
- **[2026/03/11]** **v0.2** `greyclaw` CLI によるワンクリック展開。スキルはデフォルト有効、RL はオプトイン方式に。
- **[2026/03/09]** **GreyClaw** をリリース。エージェントと話すだけで自動進化。GPU デプロイ**不要**。**API** に接続するだけ。
...
**GreyClaw は、実環境でメタ学習し進化し続けるエージェントです。**
いつも通りエージェントと話すだけで、GreyClaw はすべてのライブ会話を学習シグナルに変換し、オフライン学習だけに頼るのではなく、実際のデプロイを通じてエージェントを継続的に改善します。
...
GPU クラスタは不要です。GreyClaw は任意の OpenAI 互換 LLM API で動作し、クラウドベースの LoRA トレーニングには Tinker 互換バックエンドを利用します。[Tinker](https://www.thinkingmachines.ai/tinker/) が標準の参照パスで、必要に応じて MinT を別途互換パッケージ経由で有効化できます。

## 🤖 主な機能

### **ワンクリック展開**
`greyclaw setup` で一度設定したら、`greyclaw start` でプロキシ起動・スキル注入・OpenClaw 接続まで自動化。手動シェルスクリプト不要。
...
`rl.backend=mint` を使う場合は、同じ環境に MinT 互換パッケージを別途インストールしてください。たとえば [`mindlab-toolkit`](https://github.com/MindLab-Research/mindlab-toolkit) です。GreyClaw はこの依存をデフォルトパッケージに含めず、RL 利用者が Tinker と MinT を明示的に選べるようにしています。
...
GreyClaw の RL パスは `tinker` と `mint` を明示的に切り替えられます。推奨デフォルトは `auto` で、MinT パッケージが入っていれば Mint 形式の credentials や base URL から MinT を推定できます。
...
以上です。GreyClaw がプロキシを起動し、OpenClaw を自動設定してゲートウェイを再起動します。OpenClaw を開いてチャットを開始すると、各ターンでスキルが注入され、終了後に自動的に新しいスキルとして集約されます。

---

## ⚙️ 設定

設定ファイルは `~/.greyclaw/config.yaml` に保存され、`greyclaw setup` によって作成されます。

**CLI コマンド：**

```
greyclaw setup                  # 初回インタラクティブ設定ウィザード
greyclaw start                  # GreyClaw を起動（デフォルト: madmax モード）
greyclaw start --daemon         # GreyClaw をバックグラウンドで起動
greyclaw start --daemon --log-file /tmp/greyclaw.log  # カスタムログパス
greyclaw start --mode rl        # このセッションで RL モードを強制（スケジューラなし）
greyclaw start --mode skills_only  # このセッションでスキルのみモードを強制
greyclaw stop                   # 実行中の GreyClaw インスタンスを停止
greyclaw status                 # プロキシの状態、動作モード、スケジューラ状態を確認
greyclaw config show            # 現在の設定を表示
greyclaw config KEY VALUE       # 設定値を変更
```

`--daemon` で GreyClaw を起動すると、ローカルプロキシが正常に動作するまでコマンドは待機します。`greyclaw status` で準備状況を確認し、`greyclaw stop` でバックグラウンドプロセスを停止できます。
...
proxy:
  port: 30000
  api_key: ""              # ローカル GreyClaw プロキシ用の任意 bearer token

skills:
  enabled: true
  dir: ~/.greyclaw/skills   # スキルライブラリのディレクトリ
...
最も軽量なモードです。GPU も RL バックエンドも不要。GreyClaw が LLM をプロキシの背後に配置し、各ターンで関連スキルを注入、会話終了後に新しいスキルを自動集約します。

スキルは `~/.greyclaw/skills/` に個別の `SKILL.md` ファイルとして保存される短い Markdown 指示です。ライブラリは使用とともに自動的に成長します。
...
GreyClaw は以下のオープンソースプロジェクトの上に構築されています：
