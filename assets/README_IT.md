...
```bash
greyclaw setup              # configurazione guidata iniziale
greyclaw start              # predefinito: modalita madmax, Skill + addestramento RL programmato
greyclaw start --daemon     # esegui in background, log -> ~/.greyclaw/greyclaw.log
greyclaw start --daemon --log-file /tmp/greyclaw.log  # percorso log personalizzato
greyclaw start --mode rl    # RL senza scheduler (addestra immediatamente a batch completo)
greyclaw start --mode skills_only  # solo Skill, nessun RL (Tinker non necessario)
```
...
- **[11/03/2026]** **v0.2** Deploy con un click tramite la CLI `greyclaw`. Le Skill sono attive per impostazione predefinita, l'RL e ora opzionale.
- **[09/03/2026]** Rilascio ufficiale di **GreyClaw**. Parla con il tuo agente e lascia che si evolva automaticamente. **Nessuna** GPU necessaria, basta collegarsi alla **API**.
...
**GreyClaw e un agente che meta-apprende e si evolve in scenari reali.**
Parla con il tuo agente come faresti normalmente. GreyClaw trasforma ogni conversazione in tempo reale in un segnale di apprendimento, consentendo all'agente di migliorare continuamente attraverso il deploy nel mondo reale, non solo con l'addestramento offline.
...
Nessun cluster GPU necessario. GreyClaw funziona con qualsiasi API LLM compatibile con OpenAI, e utilizza un backend compatibile con Tinker per l'addestramento LoRA nel cloud. [Tinker](https://www.thinkingmachines.ai/tinker/) e il percorso di riferimento predefinito, e MinT puo essere abilitato tramite un pacchetto di compatibilita separato quando necessario.

## 🤖 Funzionalita Principali

### **Deploy con un click**
Configura una sola volta con `greyclaw setup`, poi `greyclaw start` avvia il proxy, inietta le Skill e collega OpenClaw automaticamente. Nessuno script shell manuale necessario.
...
Se vuoi usare `rl.backend=mint`, installa separatamente il pacchetto di compatibilita MinT nello stesso ambiente, ad esempio [`mindlab-toolkit`](https://github.com/MindLab-Research/mindlab-toolkit). GreyClaw mantiene questa dipendenza fuori dal pacchetto predefinito, cosi gli utenti RL possono scegliere esplicitamente tra Tinker e MinT.
...
Il percorso RL di GreyClaw puo commutare esplicitamente tra `tinker` e `mint`. `auto` e il valore predefinito consigliato e inferira comunque MinT da credenziali o base URL in stile Mint quando il pacchetto MinT e installato.
...
Tutto qui. GreyClaw avvia il proxy, configura automaticamente OpenClaw e riavvia il gateway. Apri OpenClaw e inizia a chattare: le Skill vengono iniettate ad ogni turno, e la sessione viene automaticamente riassunta in nuove Skill al termine.

---

## ⚙️ Configurazione

La configurazione si trova in `~/.greyclaw/config.yaml`, creata da `greyclaw setup`.

**Comandi CLI:**

```
greyclaw setup                  # Procedura guidata di configurazione iniziale
greyclaw start                  # Avvia GreyClaw (predefinito: modalita madmax)
greyclaw start --daemon         # Avvia GreyClaw in background
greyclaw start --daemon --log-file /tmp/greyclaw.log  # Percorso log personalizzato
greyclaw start --mode rl        # Forza la modalita RL (senza scheduler) per questa sessione
greyclaw start --mode skills_only  # Forza la modalita solo Skill per questa sessione
greyclaw stop                   # Ferma un'istanza GreyClaw in esecuzione
greyclaw status                 # Controlla lo stato del proxy, la modalita e lo scheduler
greyclaw config show            # Visualizza la configurazione corrente
greyclaw config KEY VALUE       # Imposta un valore di configurazione
```

Quando avvii GreyClaw con `--daemon`, il comando attende che il proxy locale sia operativo prima di restituire il controllo. Usa `greyclaw status` per verificare lo stato e `greyclaw stop` per arrestare il processo in background.
...
proxy:
  port: 30000
  api_key: ""              # opzionale: bearer token per il proxy GreyClaw locale

skills:
  enabled: true
  dir: ~/.greyclaw/skills   # la tua libreria di Skill
...
La modalita piu leggera. Nessuna GPU, nessun backend RL necessario. GreyClaw posiziona il tuo LLM dietro un proxy che inietta le Skill pertinenti ad ogni turno, poi riassume automaticamente nuove Skill dopo ogni conversazione.

Le Skill sono brevi istruzioni Markdown conservate in `~/.greyclaw/skills/` come singoli file `SKILL.md`. La libreria cresce automaticamente con l'utilizzo.
...
GreyClaw si basa sui seguenti progetti open-source:
