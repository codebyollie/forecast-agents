...
```bash
greyclaw setup              # معالج الإعداد لمرة واحدة
greyclaw start              # الوضع الافتراضي: madmax، مهارات + تدريب RL مُجدوَل
greyclaw start --daemon     # تشغيل في الخلفية، السجلات -> ~/.greyclaw/greyclaw.log
greyclaw start --daemon --log-file /tmp/greyclaw.log  # مسار سجل مخصص
greyclaw start --mode rl    # RL بدون مُجدوِل (يتدرّب فورًا عند اكتمال الدُّفعة)
greyclaw start --mode skills_only  # مهارات فقط، بدون RL (لا حاجة لـ Tinker)
```
...
- **[2026/03/11]** **v0.2** نشر بنقرة واحدة عبر واجهة `greyclaw` CLI. المهارات مُفعّلة افتراضيًا، وRL أصبح اختياريًا.
- **[2026/03/09]** إطلاق **GreyClaw** رسميًا. فقط تحدّث مع الوكيل ودعه يتطوّر تلقائيًا. **لا حاجة** لنشر GPU، فقط اتصل بـ **API**.
...
**GreyClaw وكيل يتعلّم فوقيًا ويتطوّر في البيئات الحقيقية.**
فقط تحدّث مع وكيلك كالمعتاد. يحوّل GreyClaw كل محادثة حيّة إلى إشارة تعلّم، مما يُمكّن الوكيل من التحسّن المستمر من خلال النشر الفعلي بدلًا من الاعتماد على التدريب دون اتصال فقط.

في الخلفية، يضع GreyClaw نموذجك خلف وكيل وسيط متوافق مع OpenAI (مع نقطة نهاية `/v1/messages` متوافقة مع Anthropic للوكلاء مثل NanoClaw) يعترض التفاعلات من OpenClaw وNanoClaw وNemoClaw وغيرها من الوكلاء المدعومين، ويحقن المهارات ذات الصلة في كل دور، ويتعلّم فوقيًا من التجارب المتراكمة. تُلخَّص المهارات تلقائيًا بعد كل جلسة. عند تفعيل RL، يؤجّل مُجدوِل التعلّم الفوقي تحديثات الأوزان إلى فترات الخمول حتى لا يُقاطَع الوكيل أثناء الاستخدام النشط.

لا حاجة لمجموعة GPU. يعمل GreyClaw مع أي واجهة LLM API متوافقة مع OpenAI مباشرةً، ويستخدم واجهة خلفية متوافقة مع Tinker لتدريب LoRA السحابي. [Tinker](https://www.thinkingmachines.ai/tinker/) هو المسار المرجعي الافتراضي، ويمكن تفعيل MinT من خلال حزمة توافق منفصلة عند الحاجة.

## 🤖 الميزات الرئيسية

### **نشر بنقرة واحدة**
قم بالإعداد مرة واحدة باستخدام `greyclaw setup`، ثم `greyclaw start` يُشغّل الوكيل الوسيط ويحقن المهارات ويربط OpenClaw تلقائيًا. لا حاجة لسكربتات shell يدوية.
...
إذا كنت تريد استخدام `rl.backend=mint`، قم بتثبيت حزمة توافق MinT بشكل منفصل في نفس البيئة، مثل [`mindlab-toolkit`](https://github.com/MindLab-Research/mindlab-toolkit). لا يضمّن GreyClaw هذه التبعية في الحزمة الافتراضية حتى يتمكّن مستخدمو RL من اختيار Tinker أو MinT بشكل صريح.
...
يمكن لمسار RL في GreyClaw التبديل صراحةً بين `tinker` و`mint`. القيمة الافتراضية الموصى بها هي `auto` وستظل تستنتج MinT من بيانات الاعتماد أو عناوين URL ذات النمط المشابه لـ Mint عندما تكون حزمة MinT مثبّتة.
...
هذا كل شيء. يُشغّل GreyClaw الوكيل الوسيط ويُعدّ OpenClaw تلقائيًا ويُعيد تشغيل البوابة. افتح OpenClaw وابدأ المحادثة، حيث تُحقن المهارات في كل دور وتُلخَّص الجلسة تلقائيًا إلى مهارات جديدة عند الانتهاء.

---

## ⚙️ الإعدادات

ملف الإعدادات موجود في `~/.greyclaw/config.yaml`، يُنشأ بواسطة `greyclaw setup`.

**أوامر CLI:**

```
greyclaw setup                  # معالج الإعداد التفاعلي لأول مرة
greyclaw start                  # تشغيل GreyClaw (الوضع الافتراضي: madmax)
greyclaw start --daemon         # تشغيل GreyClaw في الخلفية
greyclaw start --daemon --log-file /tmp/greyclaw.log  # مسار سجل مخصص
greyclaw start --mode rl        # فرض وضع RL لهذه الجلسة (بدون مُجدوِل)
greyclaw start --mode skills_only  # فرض وضع المهارات فقط لهذه الجلسة
greyclaw stop                   # إيقاف مثيل GreyClaw قيد التشغيل
greyclaw status                 # التحقق من صحة الوكيل الوسيط والوضع الحالي وحالة المُجدوِل
greyclaw config show            # عرض الإعدادات الحالية
greyclaw config KEY VALUE       # تعيين قيمة إعداد
```

عند تشغيل GreyClaw مع `--daemon`، ينتظر الأمر حتى يصبح الوكيل المحلي جاهزًا قبل الإرجاع. استخدم `greyclaw status` للتحقق من الجاهزية و `greyclaw stop` لإيقاف العملية في الخلفية.
...
proxy:
  port: 30000
  api_key: ""              # اختياري: رمز bearer للوكيل الوسيط المحلي لـ GreyClaw

skills:
  enabled: true
  dir: ~/.greyclaw/skills   # دليل مكتبة المهارات الخاصة بك
...
أخف وضع تشغيل. لا حاجة لـ GPU أو واجهة RL خلفية. يضع GreyClaw واجهة LLM الخاصة بك خلف وكيل وسيط يحقن المهارات ذات الصلة في كل دور محادثة، ثم يُلخّص مهارات جديدة تلقائيًا بعد كل محادثة.

المهارات هي تعليمات Markdown قصيرة تُخزَّن في `~/.greyclaw/skills/` كملفات `SKILL.md` مستقلة. تنمو المكتبة تلقائيًا مع استخدامك.
...
GreyClaw مبني على المشاريع مفتوحة المصدر التالية:
